...
- Treat fallback-only `unit_died` contexts with last known HP above the completion threshold as ambiguous trash evidence, not confirmed boss kills. Keep them diagnostic-only unless boss-frame, worldboss, council, or low-HP evidence confirms the context.
