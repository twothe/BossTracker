...
- Persistent rebuild evidence is stricter than runtime learning. Only confirmed completed boss segments are stored in `BossTrackerDB.evidence.instances`; valid completion is either `unit_died` or `low_hp_completion`. Wipes, resets, high-HP partials, and ambiguous attempts remain only in bounded session-local incomplete diagnostics.
- A pull-wide evidence draft may overflow in add-heavy raids. That truncation is diagnostic by itself; completed boss components should still commit owner-scoped bounded evidence when their packed actor, spell, and event tables fit permanent limits.
- Actor evidence stores boss-context start/end anchors separately from first/last stored spell events. Rebuilds must use the context anchor for first-offset timing so a boss that is visible before its first recorded mechanic does not replay as if the mechanic happened at pull start.
...
- Phase segmentation uses HP buckets, long activation gaps, and boss-visible aura state changes. Accepted boss self-aura changes and first/last boss-applied player aura transitions create interpreted phase segments for following abilities, while the combat-log aura events remain the raw evidence. Active boss self-aura phases outrank player-aura state and HP/gap buckets for following boss abilities, so cyclic boss phases such as Chromaggus adaptations are not stolen by target debuffs. The aura boundary event itself must not be learned as an ability that occurs at the start of the phase it just created. Re-entered aura phases count as new phase-offset samples for abilities that fire once per phase entry. Permanent evidence stores only anonymous player-target flags and per-kill target slots for player aura targets so rebuilds can reproduce overlapping phase signals without syncing player identities.
- Phase timing should outrank normal interval timing only when the same interpreted phase is observed repeatedly, or when the only interval evidence is a single weak gap across one-off transition segments. Stable intervals with multiple samples should remain time-based even if HP or aura segments changed between activations. If the global interval is unstable but the same ability repeats stably inside an active boss-aura phase, the model may use a phase-local interval that is scheduled only while that phase segment is active.
- In 5-player party instances, exact single-boss models are preserved when a later group encounter variant contains the same actor. Fast dungeon chain-pulls can create valid group evidence without proving that the boss should lose its standalone model; raid phase actors still normalize into their group model.
- In raid instances, a weak boss-frame add contained inside a worldboss primary should remain separate when its own event, occurrence, and ability evidence is add-like. A substantial companion boss still groups with the primary so councils and multi-actor encounters keep their shared encounter key.
- When a timed prediction misses its learned timing window, the UI briefly marks it as overdue and then hides it until the next real activation becomes the new anchor. The delayed state is logged as bounded prediction diagnostics so later interpreters can detect missing phase, targetability, aura, or add-state conditions without polluting permanent raw evidence.
...
- Boss-applied player aura bursts use the apply/refresh wave as the timer anchor. Later damage, miss, aura removal, dose changes, dispel fallout, or short single-target refreshes remain diagnostic evidence but do not create new timer activations for that aura.
- Extremely broad interval ranges are not shown as normal time timers. The runtime countdown uses the earliest learned interval, so a model with a very large max/min spread would warn far too early and then sit overdue instead of helping the player.
- Damage-only or miss-only spell records are retained as diagnostics but hidden from timers because they lack a cast, aura, summon, heal, or interrupt anchor. Expiration fallout such as an explosion after a debuff ends should not be learned as its own boss cooldown.
- Very late low-HP casts or channels that occur only at the end of a fight are suppressed as terminal mechanics rather than learned as short repeat cooldowns.
- A single interrupted cast from one pull remains diagnostic instead of becoming a first-offset timer. Repeated evidence or stronger boss-owned context is needed before such an interruptible spell becomes player-facing.
...
- Display-floor comparisons use a small floating-point tolerance because packed 0.1-second evidence can replay exact boundaries as values a few machine epsilon below the intended threshold.
- A cast-start-backed spell with mostly displayable intervals should not be hidden because of a short raw outlier, especially when interrupts or phase retries create one sub-floor gap. Near-floor cast-start timers may also clamp to the configured display floor when the average is narrowly below it and at least one observed interval is displayable. Instant cast-success filler without cast-start evidence remains eligible for normal routine suppression.
...
- Repeated filler can still look relevant for the first two casts of a fresh pull if the initial interval is unusually long. A global routine-spell index derived from confirmed learned bosses suppresses those shared filler spells before live provisional timers are created, but only after high cross-encounter evidence. Common boss spell names must remain locally learnable when their own encounter evidence is timer-like.
- Pure aura-only repeats at nearly the same HP are hidden as likely passive, consequence, or phase-state noise unless future relevance logic has stronger evidence that they are player-actionable mechanics. Aura dose changes count as aura state in this filter. Pure boss self-aura, boss-applied player-aura, and mixed boss/player aura state are also hidden by default because they are primarily phase markers for the interpreter, not player-facing timers.
- In the Ascension Blackwing Lair Nefarian sequence, `Coward` is a run-time player debuff from taking too long to reach the boss. Stack count and duration depend on player travel time, so it should remain hidden as player aura state and must not become an HP or cooldown timer.
...
- Stable HP samples need at least three observations before they can become an `hp_gate` rule, and the ability must occur no more than once per pull. Repeated events at nearly the same HP are not threshold crossings. With one or two pulls, the model prefers timing or phase timing so normal scripted boss casts do not appear as HP percentage bars just because group DPS was similar.
...
- `Learning/RuleLearner.lua` keeps competing rule candidates such as `time_interval`, `phase_time_interval`, `first_offset`, `hp_gate`, `phase_start_offset`, `phase_once`, and `encounter_add`.
...
4. Timer rule candidates: every ability should maintain competing hypotheses such as `time_interval`, `phase_time_interval`, `first_offset`, `hp_gate`, `phase_start_offset`, `phase_once`, `conditional_retry`, and `encounter_add`. The display should use the highest-confidence user-relevant rule without showing the technical category.
