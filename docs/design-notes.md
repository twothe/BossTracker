...
- Pure aura-only repeats at nearly the same HP are hidden as likely passive, consequence, or phase-state noise unless future relevance logic has stronger evidence that they are player-actionable mechanics. Pure boss self-aura state is also hidden by default because it is primarily a phase marker for the interpreter, not a player-facing timer.
