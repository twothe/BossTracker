...
- Phase segmentation uses HP buckets, long activation gaps, and boss-visible aura state changes. Accepted boss self-aura changes and first/last boss-applied player aura transitions create interpreted phase segments, while the combat-log aura events remain the raw evidence. Re-entered aura phases count as new phase-offset samples for abilities that fire once per phase entry. Permanent evidence stores only anonymous player-target flags and per-kill target slots for player aura targets so rebuilds can reproduce overlapping phase signals without syncing player identities.
